...
export default function IssuePullRequestsPage() {
    ...
    if (!octokit) {
      ...
      const fetchData = async () => {
        try {
          ...
          const finalPR = pullRequestData.filter(
            (item: any) => item.event == "cross-referenced"
          );
          for (let i = 0; i < finalPR.length; i++) {
            setPrUrls((prev) => [
              ...prev,
              finalPR[i]?.source?.issue?.pull_request?.html_url,
            ]);
          }
          ...
          <div className="mb-8">
            <div className="flex items-center gap-4 mb-4">
              <Button
                variant="outline"
                size="sm"
                className="flex dark:bg-transparent dark:text-white  bg-white text-black"
              >
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <p className="text-muted-foreground">
                  Pull requests linked to issue #{issueNumber} in {owner}/
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <CardTitle className="text-lg dark:text-neutral-100 text-neutral-900">
                          {issue?.issue_name}
                        </CardTitle>
                        {issue?.rewardAmount && (
                          <Badge
                      ...
                      <CardDescription className="dark:text-neutral-100 text-neutral-900">
                        Issue #{issue?.project_issues} • Opened by{" "}
                        {issue?.publisher}{" "}
                        {issue?.issue_date &&
                          `on ${new Date(
                            issue.issue_date
                      ...
                      <p className="text-sm dark:text-neutral-100 text-neutral-900 line-clamp-2">
                      ...
                      <CardHeader>
                        <div className="flex items-start justify-between">
                          <div className="space-y-2 flex-1">
                            <div className="flex justify-between  items-center gap-2">
                              ...
                              <div>
                                <a
                                  ...
                                  target="_blank"
                                  rel=""
                                  className="flex items-center px-5 py-2 border border-neutral-300 rounded-md font-medium hover:dark:text-black hover:dark:bg-white  transition"
                                >
                                  <svg
                            ...
                            <CardDescription>
                              Pull Request #{pr.source.issue.number} •{" "}
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="border-t pt-4 flex justify-between items-center">
                          <div className="text-sm text-muted-foreground">
                            This pull request addresses issue #{issueNumber}:{" "}
                            {issue?.project_repository}
                          </div>
                          <div>
                            <Link
          ...
          {pullRequests.length === 0 && (
            <div className="text-center py-12">
              <div className="space-y-2">
                <p className="text-muted-foreground">
                  No pull requests are linked to this issue yet.
                </p>
                <p className="text-sm text-muted-foreground">
                  Pull requests that reference issue #{issueNumber} will appear
                  here.
                </p>
              </div>
            </div>
