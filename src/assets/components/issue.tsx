    ...
    const fetchLikes = async () => {
      try {
        const response = await fetch(
          `/api/likes?userId=${activeUser}&projectName=${Project}`,
          {
            method: "GET",
            headers: {
        ...
        if (data && data.projects) {
          ...
          setLiked(
            data.projects.some((like: Likes) => like.userId === activeUser)
          );
        }
      } catch (error) {
  ...
  return (
    <div className="relative h-full rounded-xl border md:rounded-xl  cursor-pointer transition-all duration-300">
      ...
      <CardHeader className="p-0">
        <div className="relative h-48 w-full overMonad-hidden rounded-t-lg">
          <img
            src={image}
            alt={Project}
      ...
      <CardContent className="p-4">
        <div className="flex justify-between items-start mb-2">
          ...
          <div className="flex">
            <div
              className="flex px-1 text-neutral-400 cursor-pointer"
          ...
          {languages &&
            ...
            Object.keys(languages).map((language: string) => {
              ...
              return (
                <div
                  key={language}
                  className="inline-flex text-[20px] text-white py-1"
                >
                  {iconName && (
                    <Icon
                      icon={iconName}
