...
export default function PricingPlans() {
  ...
  return (
    <section className="w-full text-white flex items-center justify-center py-5 mx-auto">
      {/* Mobile Carousel */}
      <div className="md:hidden w-full overMonad-hidden relative rounded-xl">
        <div
          ...
          {plans.map((plan, index) => (
            ...
            >
              <div className="flex flex-col gap-4">
                ...
                <p className="text-sm whitespace-pre-line font-medium text-neutral-400">
                  {plan.description}
                </p>
          ...
          {plans.map((_, index) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all ${
                currentIndex === index ? "bg-white" : "bg-neutral-500"
              }`}
            />
          ))}
        </div>
...
export function PricingTabs() {
  const tabs = [
    {
      ...
      value: "Contributer",
      content: (
        <div className="w-full overMonad-hidden relative min-h-[60vh] rounded-2xl py-10 px-5 text-xl md:text-4xl font-bold text-white bg-[#9D00FF] flex flex-col items-center gap-3">
          <h1 className="text-[5vw] md:text-[3.3vw] font-bold text-center">
            Ready to Turn Code into Money?
          </h1>
          <p className="md:text-[1vw] text-[2vw] font-semibold text-center">
            Join a growing community of developers monetizing their open-source
            passion.
          </p>
    ...
    },
    {
      ...
      content: (
        <div className="w-full overMonad-hidden relative min-h-[60vh] rounded-2xl py-10 px-5 text-xl md:text-4xl font-bold text-white bg-[#9D00FF] flex flex-col items-center gap-3">
          <h1 className="text-[4.7vw] md:text-[3.3vw] font-bold text-center">
            Boost Your Project, Attract Top Talent
          </h1>
          <p className="md:text-[1vw] text-[2vw] font-semibold text-center">
            Create Bounties & Grow Your Community.
          </p>
  ...
  return (
    <div className="min-h-[22rem] md:min-h-[42rem] [perspective:1000px] relative b flex flex-col max-w-[90vw] md:max-w-[80vw] mx-auto w-full items-start justify-start  md:my-[13vh] z-20">
      <Tabs tabs={tabs} />
    </div>
  );
}
