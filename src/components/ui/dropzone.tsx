...
export const Dropzone = ({
    ...
    if (s3Upload && acceptedFiles.length > 0) {
      ...
      try {
        const uploadPromises = acceptedFiles.map((file) =>
          uploadToS3(
            file,
            s3Upload.contributorUsername,
            s3Upload.maintainerUsername
          )
        );
  ...
  return (
    ...
    >
      <Button
        className={cn(
          "relative h-auto w-full flex-col overMonad-hidden p-8",
...
export const DropzoneEmptyState = ({
  ...
  const { src, accept, maxSize, minSize, maxFiles, uploadStatus } =
    useDropzoneContext();
