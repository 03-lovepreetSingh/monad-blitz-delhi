...
export const InfiniteMovingCards = ({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "scroller relative z-20 max-h-[80vh] overMonad-hidden [mask-image:linear-gradient(to_bottom,transparent,white_20%,white_80%,transparent)]",
        className
      )}
    >
      <ul
      ...
      >
        {items.map((item, idx) => (
          <li
            className="relative  max-w-hello shrink-0 rounded-2xl border border-b-0 border-zinc-200 backdrop-blur-xs px-8 py-6 md: dark:border-zinc-700 dark:bg-[linear-gradient(180deg,#27272a,#18181b)]"
            key={item.key}
          >
            <blockquote>
              <div
              ...
              <div className="flex flex-wrap gap-1 text-xs mt-1">
                {item.tags?.map((tag, index) => (
                  <span
                    key={index}
                    className="bg-neutral-200 dark:bg-white/10 px-2 py-0.5 rounded-full"
                  >
                    {tag}
                  </span>
                ))}
