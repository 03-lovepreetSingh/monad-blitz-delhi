...
export default function ViewScheduleClient({
  scheduleId,
}: {
  scheduleId: string;
}) {
  ...
  // Handle schedule completion and smart contract interaction
  const handleScheduleCompletion = async () => {
    if (!user) return;

    try {
      addToast("Schedule completed! Processing reward...", "success");

      // Call smart contract withdraw function
      // You'll need to provide the appropriate parameters based on your contract
      // This is a placeholder - adjust parameters as needed
      if (withdrawWrite) {
        await withdrawWrite();
  ...
  useEffect(() => {
    if (schedule && calculateProgress() === 100) {
      handleScheduleCompletion();
    }
  }, [schedule, user, withdrawWrite]);

  const fetchSchedule = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `https://learnfast-backend.onrender.com/api/schedules/detail/${scheduleId}`,
        {
          headers: {
  ...
  const handleVideoStatusChange = async (
    ...
    try {
      ...
      const response = await fetch(
        `https://learnfast-backend.onrender.com/api/schedules/${scheduleId}/progress`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      updatedSchedule.schedule_data[dayIndex].videos[videoIndex].completed =
        completed;
  ...
  const filterVideos = (videos: Video[]) => {
    return videos.filter((video) => {
      ...
      const matchesCompletion =
        filterCompleted === null || video.completed === filterCompleted;
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="text-sm text-gray-500">
                        {day.videos.filter((v) => v.completed).length} /{" "}
                        {day.videos.length} completed
                      </div>
                      <motion.div
                        animate={{
                          rotate: expandedDays.includes(day.day) ? 180 : 0,
                        }}
              ...
              >
                ...
                >
                  ...
                  <div className="relative pt-[56.25%]">
                    <iframe
                      src={`${selectedVideo.link.replace(
                        "watch?v=",
                        "embed/"
                      )}?enablejsapi=1`}
